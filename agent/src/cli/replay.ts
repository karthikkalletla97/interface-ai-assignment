...
function firstArtifact(dir: string): string {
  ...
  if (files.length > 1) {
    throw new Error(
      `Multiple artifacts in ${dir}:\n` +
        files.map((f) => `  - ${f}`).join("\n") +
        `\nChoose one with ARTIFACT=artifacts/<file>.json (there is more than one capability).`,
    );
  }
...
async function main() {
  ...
  const artifactPath =
    process.env.ARTIFACT || firstArtifact(path.join(repoRoot, "artifacts"));
  ...
  const shownMember =
    inputs.memberId === "" ? "(empty)" : (inputs.memberId ?? "(none)");
  ...
  console.log(`mode:       approveRisky=${approveRisky}  handoff=${handoff}`);
  const result = await replay.replay(artifact, inputs, {
    approveRisky,
    handoff,
  });
  ...
  if (result.status === "success") {
    ...
    if (result.interventions?.length) {
      console.log(
        `handoffs:   ${result.interventions.length} (a human took control mid-run)`,
      );
      for (const iv of result.interventions) {
        console.log(
          `  step ${iv.step} "${iv.control}" -> human; page changed: ${iv.changed}`,
        );
      }
    }
  }
  ...
  if (result.status === "escalated") {
    ...
    console.log(
      `escalation: step ${e.step} (${e.action} "${e.control}") - ${e.reason}`,
    );
    console.log(
      "            re-run with APPROVE_RISKY=1 to auto-authorize, or HANDOFF=1 for a live human handoff.",
    );
  }
  if (result.status === "hard_failure") {
    const f = result.failure;
    console.log(
      `failure:    step ${f.step} (${f.action}); expected ${f.expected}; observed ${f.observed}${result.escalate ? "  [ESCALATE]" : ""}`,
    );
  }
